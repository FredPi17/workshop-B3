...
function MarkerLabel_(marker, crossURL, handCursorURL) {
    this.marker_ = marker, this.handCursorURL_ = marker.handCursorURL, this.labelDiv_ = document.createElement("div"), this.labelDiv_.style.cssText = "position: absolute; overflow: hidden;", this.crossDiv_ = MarkerLabel_.getSharedCross(crossURL)
}
...
function centerAllMarkers(settings) {
    for (var bounds = new google.maps.LatLngBounds, i = 0; i < settings._markers.length; i++) bounds.extend(settings._markers[i].getPosition());
    settings.map.fitBounds(bounds), $(window).width() > 900 ? settings.map.panBy(200, 0) : $(window).width() > 550 && settings.map.panBy(150, 0)
}

function refreshSelectedMarker(settings) {
    for (var i = 0; i < settings._markers.length; i++) settings._markers[i].labelClass = "map-label", settings._markers[i].setIcon(settings.markerTypes.marker_violet)
}

function afficheInfos(aCampus, settings, container) {
    settings.oInfosCampusVides || (settings.oInfosCampusVides = container.find(".infos-map .content-campus").clone());
    var oInfosCampus = settings.oInfosCampusVides.clone(), szAdresseHTML = "";
    void 0 != aCampus.adresse1 && "" != aCampus.adresse1 && (szAdresseHTML += aCampus.adresse1), void 0 != aCampus.adresse2 && "" != aCampus.adresse2 && (szAdresseHTML = szAdresseHTML + "<br/>" + aCampus.adresse2), void 0 != aCampus.code_postal && "" != aCampus.code_postal && (szAdresseHTML = szAdresseHTML + "<br/>" + aCampus.code_postal + " "), szAdresseHTML += aCampus.ville;
    var szTelHTML = "";
    void 0 != aCampus.tel && "" != aCampus.tel && (szTelHTML = "T̩l : " + szTelHTML + aCampus.tel), void 0 != aCampus.fax && "" != aCampus.fax && (szTelHTML = szTelHTML + "<br/>Fax : " + aCampus.fax), oInfosCampus.find(".campus-title").html("Campus <br/>" + aCampus.nom), oInfosCampus.find(".campus-adresse").html(szAdresseHTML), oInfosCampus.find(".campus-tel").html(szTelHTML), oInfosCampus.find(".campus-mail").html(aCampus.mail), oInfosCampus.find(".campus-mail").attr("href", "mailto:" + aCampus.mail), oInfosCampus.find(".campus-lien").attr("href", aCampus.lien), container.find(".infos-map .content-campus").html(oInfosCampus), container.find(".infos-map").removeClass("content-default"), container.find(".infos-map").fadeIn()
}

function gestion_menu() {
    $("#menu-h > ul > li").unbind("mouseenter mouseleave"), $("#menu-h > ul > li > a").unbind("click"), $(window).width() > 880 ? $("#menu-h > ul > li").hover(function (event) {
        $(this).find(".sub-menu").fadeIn()
    }, function (event) {
        $(this).find(".sub-menu").stop().fadeOut()
    }) : $("#menu-h > ul > li > a").click(function (event) {
        event.preventDefault(), $(this).parent("li").hasClass("ouvert") ? ferme_sousmenu_mobile($(this).parent("li")) : ouvre_sousmenu_mobile($(this).parent("li"))
    })
}

function ferme_sousmenu_mobile(li) {
    li.removeClass("ouvert"), li.find(".sub-menu").slideUp()
}

function ouvre_sousmenu_mobile(li) {
    li.addClass("ouvert"), li.find(".sub-menu").slideDown()
}

function gestion_ouverture_menu() {
    $("#btn-menu").click(function () {
        $(this).hasClass("ouvert") ? ferme_menu() : ouvre_menu()
    }), $(".fond-violet").click(function () {
        ferme_menu()
    })
}

function ouvre_menu() {
    $(".fond-violet").fadeIn(), $("#zone-menu").addClass("ouvert"), $("#btn-menu").addClass("ouvert")
}

function ferme_menu() {
    $(".fond-violet").fadeOut(), $("#zone-menu").removeClass("ouvert"), $("#btn-menu").removeClass("ouvert")
}

function initializeOwlInfos() {
    $(".slider-infos").owlCarousel({
        nav: !0,
        loop: !0,
        autoWidth: !0,
...
function check_slider_infos() {
    $(window).width() < 880 ? ($(".slider-infos").addClass("owl-carousel"), initializeOwlInfos()) : ($(".slider-infos").removeClass("owl-carousel"), $(".slider-infos").trigger("destroy.owl.carousel"))
}

function initializeOwlMasonry() {
    $(".masonry").owlCarousel({
        nav: !0,
        loop: !0,
        autoWidth: !0,
...
function check_slider_masonry() {
    $(window).width() < 880 ? ($(".masonry").addClass("owl-carousel"), initializeOwlMasonry()) : ($(".masonry").removeClass("owl-carousel"), $(".masonry").trigger("destroy.owl.carousel"))
}
...
!function (a, b) {
    ...
    $.fn.dragScroll = function (options) {
        function doMousedown(e) {
            e.preventDefault(), down = !0, x = e.pageX, y = e.pageY, top = $(this).scrollTop(), left = $(this).scrollLeft()
        }

        var x, y, top, left, down, $scrollArea = $(this);
        $($scrollArea).attr("onselectstart", "return false;"), $($scrollArea).mousedown(function (e) {
            if ("object" == typeof options.limitTo) for (var i = 0; i < options.limitTo.length; i++) $(e.target).hasClass(options.limitTo[i]) && doMousedown(e); else doMousedown(e)
        }), $($scrollArea).mouseleave(function (e) {
            down = !1
        }), $("body").mousemove(function (e) {
            if (down) {
                var newX = e.pageX, newY = e.pageY;
                $($scrollArea).scrollTop(top - newY + y), $($scrollArea).scrollLeft(left - newX + x)
            }
        }), $("body").mouseup(function (e) {
            down = !1
        })
    }
}(jQuery), inherits(MarkerLabel_, google.maps.OverlayView), MarkerLabel_.getSharedCross = function (crossURL) {
    var div;
    ...
    var cSavedZIndex, cLatOffset, cLngOffset, cIgnoreClick, cRaiseEnabled, cStartPosition, cStartCenter, me = this,
        cMouseIsDown = !1, cDraggingLabel = !1, cRaiseOffset = 20, cDraggingCursor = "url(" + this.handCursorURL_ + ")",
    ...
    this.getPanes().markerLayer.appendChild(this.labelDiv_), "undefined" == typeof MarkerLabel_.getSharedCross.processed && (this.getPanes().markerLayer.appendChild(this.crossDiv_), MarkerLabel_.getSharedCross.processed = !0), this.listeners_ = [google.maps.event.addDomListener(this.labelDiv_, "mouseover", function (e) {
    ...
    var i;
    for (this.labelDiv_.parentNode.removeChild(this.labelDiv_), i = 0; i < this.listeners_.length; i++) google.maps.event.removeListener(this.listeners_[i])
}, MarkerLabel_.prototype.draw = function () {
    this.setContent(), this.setTitle(), this.setStyles()
}, MarkerLabel_.prototype.setContent = function () {
    var content = this.marker_.get("labelContent");
    "undefined" == typeof content.nodeType ? this.labelDiv_.innerHTML = content : (this.labelDiv_.innerHTML = "", this.labelDiv_.appendChild(content))
}, MarkerLabel_.prototype.setTitle = function () {
    this.labelDiv_.title = this.marker_.getTitle() || ""
}, MarkerLabel_.prototype.setStyles = function () {
    var i, labelStyle;
    this.labelDiv_.className = this.marker_.get("labelClass"), this.labelDiv_.style.cssText = "", labelStyle = this.marker_.get("labelStyle");
    ...
    "undefined" == typeof yOffset && (yOffset = 0), this.labelDiv_.style.left = Math.round(position.x) + "px", this.labelDiv_.style.top = Math.round(position.y - yOffset) + "px", this.setZIndex()
}, MarkerLabel_.prototype.setZIndex = function () {
    var zAdjust = this.marker_.get("labelInBackground") ? -1 : 1;
    "undefined" == typeof this.marker_.getZIndex() ? this.labelDiv_.style.zIndex = parseInt(this.labelDiv_.style.top, 10) + zAdjust : this.labelDiv_.style.zIndex = this.marker_.getZIndex() + zAdjust
    ...
    $.fn.loadMoreAjax = function (options) {
        var settings = $.extend({callbackResponse: !1}, options),
            settingsInline = $.parseJSON($(this).parent().find(".lma-data").html());
        return settings = $.extend(settings, settingsInline), $(this).click(function () {
            var container = $(this).closest(settings.parent).find(settings.target), button = $(this);
            $.ajax({
                method: "POST",
                url: settings.ajax_url,
                data: {action: settings.action, data: settings.data}
            }).done(function (response) {
                response = $.parseJSON(response), settings.callbackResponse && settings.callbackResponse(response, settings, container, button)
            })
        }), this
    }
}(jQuery), function ($) {
    $.fn.mapify = function (options) {
        ...
        for (settings.map = new google.maps.Map(settings.mapContainer, {
            mapTypeControl: settings.mapTypeControl,
            scrollwheel: settings.scrollwheel,
            draggable: settings.draggable,
            zoom: settings.zoom
        }), settings.styles && settings.map.setOptions({styles: settings.styles}), i = 0; i < settings.markers.length; i++) {
            var szLatLng = {lat: settings.markers[i].lat, lng: settings.markers[i].lng};
            settings.markers[i].marker.map = settings.map, settings.markers[i].marker.position = szLatLng, settings.markers[i].marker.icon = settings.markerTypes[settings.markers[i].marker.icon], settings._markers[i] = new MarkerWithLabel(settings.markers[i].marker), google.maps.event.addListener(settings._markers[i], "click", function (settings, i, container) {
                return function () {
                    settings.clickCallback && settings.clickCallback(settings, i, container)
                }
            }(settings, i, this))
        }
        return settings.afterInitCallback && settings.afterInitCallback(settings, this), this
    }
    ...
    Video.Defaults = {video: !1, videoHeight: !1, videoWidth: !1}, Video.prototype.fetch = function (target, item) {
        ...
        var html, target = $(event.target), item = target.closest("." + this._core.settings.itemClass),
    ...
    function Parallax(element, options) {
        var self = this;
        "object" == typeof options && (delete options.refresh, delete options.render, $.extend(this, options)), this.$element = $(element), !this.imageSrc && this.$element.is("img") && (this.imageSrc = this.$element.attr("src"));
        var positions = (this.position + "").toLowerCase().match(/\S+/g) || [];
        if (positions.length < 1 && positions.push("center"), 1 == positions.length && positions.push(positions[0]), "top" != positions[0] && "bottom" != positions[0] && "left" != positions[1] && "right" != positions[1] || (positions = [positions[1], positions[0]]), this.positionX != undefined && (positions[0] = this.positionX.toLowerCase()), this.positionY != undefined && (positions[1] = this.positionY.toLowerCase()), self.positionX = positions[0], self.positionY = positions[1], "left" != this.positionX && "right" != this.positionX && (isNaN(parseInt(this.positionX)) ? this.positionX = "center" : this.positionX = parseInt(this.positionX)), "top" != this.positionY && "bottom" != this.positionY && (isNaN(parseInt(this.positionY)) ? this.positionY = "center" : this.positionY = parseInt(this.positionY)), this.position = this.positionX + (isNaN(this.positionX) ? "" : "px") + " " + this.positionY + (isNaN(this.positionY) ? "" : "px"), navigator.userAgent.match(/(iPod|iPhone|iPad)/)) return this.imageSrc && this.iosFix && !this.$element.is("img") && this.$element.css({
        ...
        0 == slider.length ? this.$slider = $("<img />").prependTo(this.$mirror) : (this.$slider = slider.prependTo(this.$mirror), sliderExisted = !0), this.$mirror.addClass("parallax-mirror").css({
            visibility: "hidden",
            zIndex: this.zIndex,
            position: "fixed",
            top: 0,
            ...
            self.naturalHeight && self.naturalWidth || (self.naturalHeight = this.naturalHeight || this.height || 1, self.naturalWidth = this.naturalWidth || this.width || 1), self.aspectRatio = self.naturalWidth / self.naturalHeight, Parallax.isSetup || Parallax.setup(), Parallax.sliders.push(self), Parallax.isFresh = !1, Parallax.requestRender()
    ...
    function Plugin(option) {
        return this.each(function () {
            ...
            this == window || this == document || $this.is("body") ? Parallax.configure(options) : $this.data("px.parallax") ? "object" == typeof option && $.extend($this.data("px.parallax"), options) : (options = $.extend({}, $this.data(), options), $this.data("px.parallax", new Parallax(this, options))), "string" == typeof option && ("destroy" == option ? Parallax.destroy(this) : Parallax[option]())
        })
    }
    ...
    !function () {
        ...
        refresh: function () {
            ...
            var winHeight = Parallax.winHeight, docHeight = Parallax.docHeight,
                maxOffset = Math.min(this.boxOffsetTop, docHeight - winHeight),
                minOffset = Math.max(this.boxOffsetTop + this.boxHeight - winHeight, 0),
                imageHeightMin = this.boxHeight + (maxOffset - minOffset) * (1 - this.speed) | 0,
                imageOffsetMin = (this.boxOffsetTop - maxOffset) * (1 - this.speed) | 0;
            if (imageHeightMin * this.aspectRatio >= this.boxWidth) {
                this.imageWidth = imageHeightMin * this.aspectRatio | 0, this.imageHeight = imageHeightMin, this.offsetBaseTop = imageOffsetMin;
                var margin = this.imageWidth - this.boxWidth;
                "left" == this.positionX ? this.offsetLeft = 0 : "right" == this.positionX ? this.offsetLeft = -margin : isNaN(this.positionX) ? this.offsetLeft = -margin / 2 | 0 : this.offsetLeft = Math.max(this.positionX, -margin)
            } else {
        ...
        render: function () {
            var scrollTop = Parallax.scrollTop, scrollLeft = Parallax.scrollLeft,
                overScroll = this.overScrollFix ? Parallax.overScroll : 0,
                scrollBottom = scrollTop + Parallax.winHeight;
            this.boxOffsetBottom > scrollTop && this.boxOffsetTop <= scrollBottom ? (this.visibility = "visible", this.mirrorTop = this.boxOffsetTop - scrollTop, this.mirrorLeft = this.boxOffsetLeft - scrollLeft, this.offsetTop = this.offsetBaseTop - this.mirrorTop * (1 - this.speed)) : this.visibility = "hidden", this.$mirror.css({
        ...
        setup: function () {
            if (!this.isReady) {
                var $doc = $(document), $win = $(window), loadDimensions = function () {
                    Parallax.winHeight = $win.height(), Parallax.winWidth = $win.width(), Parallax.docHeight = $doc.height(), Parallax.docWidth = $doc.width()
                }, loadScrollPosition = function () {
                    var winScrollTop = $win.scrollTop(), scrollTopMax = Parallax.docHeight - Parallax.winHeight,
                        scrollLeftMax = Parallax.docWidth - Parallax.winWidth;
                    Parallax.scrollTop = Math.max(0, Math.min(scrollTopMax, winScrollTop)), Parallax.scrollLeft = Math.max(0, Math.min(scrollLeftMax, $win.scrollLeft())), Parallax.overScroll = Math.max(winScrollTop - scrollTopMax, Math.min(winScrollTop, 0))
                };
                $win.on("resize.px.parallax load.px.parallax", function () {
                    loadDimensions(), Parallax.isFresh = !1, Parallax.requestRender()
        ...
        requestRender: function () {
            var self = this;
            this.isBusy || (this.isBusy = !0, window.requestAnimationFrame(function () {
        ...
        destroy: function (el) {
            ...
            $(el).data("px.parallax", !1), 0 === this.sliders.length && ($(window).off("scroll.px.parallax resize.px.parallax load.px.parallax"), this.isReady = !1, Parallax.isSetup = !1)
        }
    });
    var old = $.fn.parallax;
    $.fn.parallax = Plugin, $.fn.parallax.Constructor = Parallax, $.fn.parallax.noConflict = function () {
        return $.fn.parallax = old, this
    }, $(document).on("ready.px.parallax.data-api", function () {
        $('[data-parallax="scroll"]').parallax()
    })
}(jQuery, window, document), $(document).ready(function () {
    $(".accordeon-title").length > 0 && $(".accordeon-title").click(function () {
        $(this).hasClass("ouvert") ? ($(this).removeClass("ouvert"), $(this).next(".accordeon-content").slideToggle()) : ($(this).addClass("ouvert"), $(this).next(".accordeon-content").slideDown())
    })
}), $(document).ready(function () {
    $(".magnetic .picto-wrapper").click(function (e) {
        e.preventDefault(), $(this).parent(".un-cta").toggleClass("ouvert")
    })
});
var styles = [{
    ...
    stylers: [{weight: "1.76"}, {lightness: "57"}]
}, {
    featureType: "administrative.country",
    elementType: "labels.text.fill",
    ...
    stylers: [{visibility: "on"}, {saturation: "-100"}, {lightness: "68"}, {gamma: "1.11"}]
    ...
    featureType: "road",
    elementType: "all",
    stylers: [{saturation: "-100"}, {lightness: "18"}]
    ...
    featureType: "water",
    elementType: "geometry.fill",
    stylers: [{color: "#cfccd9"}]
}, {featureType: "water", elementType: "labels.text.fill", stylers: [{color: "#727272"}]}];
jQuery(".map-wrapper").each(function () {
    for (var dataSource = jQuery.parseJSON(jQuery(this).find(".map-data").html()), mapOptions = $.extend({
        mapContainer: jQuery(this).find(".map-container")[0],
        styles: styles,
        zoom: 5,
        zoomMarker: 13,
        markerTypes: {
            marker_violet: {
                path: "M0,1.89,7.65,0,15.3,1.89,7.65,9.6Z",
                fillColor: "#4b3c85",
                fillOpacity: 1,
        ...
        clickCallback: function (settings, i, container) {
            refreshSelectedMarker(settings), settings._markers[i].setIcon(settings.markerTypes.marker_jaune), settings._markers[i].labelClass = "map-label-selected", settings.map.setZoom(10), settings.map.setZoom(settings.zoomMarker), settings.map.setCenter(settings._markers[i].getPosition()), jQuery(window).width() > 900 ? settings.map.panBy(200, 0) : jQuery(window).width() > 550 ? settings.map.panBy(150, 0) : settings.map.panBy(0, 200), afficheInfos(settings.markers[i], settings, container)
        },
        afterInitCallback: function (settings, container) {
            centerAllMarkers(settings), refreshSelectedMarker(settings), container.find(".infos-map").addClass("content-default"), container.find(".icon-fermer").click(function () {
                $(window).width() <= 550 && container.find(".infos-map").fadeOut(), container.find(".infos-map").addClass("content-default"), refreshSelectedMarker(settings), centerAllMarkers(settings)
            }), settings.autoEnableMarker && settings.clickCallback(settings, 0, container)
        }
    }, dataSource), i = 0; i < mapOptions.markers.length; i++) mapOptions.markers[i].marker = {
        draggable: !1,
        icon: "marker_violet",
        labelContent: "",
        ...
        labelInBackground: !1
    }, mapOptions.markers[i].marker.labelContent = mapOptions.markers[i].nom;
    jQuery(window).width() <= 550 && (mapOptions.draggable = !1, mapOptions.scrollwheel = !1), jQuery(this).mapify(mapOptions)
}), jQuery(window).resize(function () {
    jQuery(".map-wrapper").each(function () {
        console.log(jQuery(this))
    })
}), $(document).ready(function () {
    if (gestion_menu(), gestion_ouverture_menu(), $(window).resize(function () {
        gestion_menu()
    }), $("iframe").length > 0) for (var iframes = document.getElementsByTagName("iframe"), key = /http:\/\/formulairescrm.groupe-cd.fr/, i = 0; i != iframes.length; ++i) {
        var match = iframes[i].src.search(key);
        match != -1 && $(iframes[i]).on("load", function () {
            $("html, body").scrollTop(0), setTimeout(function () {
    ...
    $(".onglets-wrapper").each(function () {
        $(".onglet-title:first-of-type").addClass("ouvert"), $(this).dragScroll({}), $(this).scroll(function () {
            nScrollLeft = $(this).scrollLeft(), $(this).find(".onglet-content").css("left", nScrollLeft)
        })
    }), $(".onglet-title").length > 0 && ($(".onglet-title").click(function () {
        $(this).hasClass("ouvert") || ($(this).parent(".onglets-wrapper").find(".onglet-title").removeClass("ouvert"), $(this).addClass("ouvert"), $(this).parent(".onglets-wrapper").find(".onglet-content").hide(), $(this).next(".onglet-content").show())
    }), $("body.iphone").length > 0 && $(".onglet-title").each(function () {
        $(this).css("min-width", $(this).find(".lien").outerWidth())
    }))
}), $(".share").length > 0 && (!function (d, s, id) {
    var js, fjs = d.getElementsByTagName(s)[0];
    d.getElementById(id) || (js = d.createElement(s), js.id = id, js.src = "//connect.facebook.net/fr_FR/sdk.js#xfbml=1&version=v2.8", fjs.parentNode.insertBefore(js, fjs))
}(document, "script", "facebook-jssdk"), window.twttr = function (d, s, id) {
    var js, fjs = d.getElementsByTagName(s)[0], t = window.twttr || {};
    return d.getElementById(id) ? t : (js = d.createElement(s), js.id = id, js.src = "https://platform.twitter.com/widgets.js", fjs.parentNode.insertBefore(js, fjs), t._e = [], t.ready = function (f) {
        t._e.push(f)
    }, t)
}(document, "script", "twitter-wjs"), window.___gcfg = {lang: "fr"}, function () {
    var po = document.createElement("script");
    po.type = "text/javascript", po.async = !0, po.src = "https://apis.google.com/js/platform.js";
    var s = document.getElementsByTagName("script")[0];
    s.parentNode.insertBefore(po, s)
}()), $(document).ready(function () {
    $(".slider-campus").length > 0 && $(".slider-campus").owlCarousel({
        nav: !0,
        loop: !0,
        autoWidth: !0,
        navText: ['<i class="icon-fleche-gauche"></i>', '<i class="icon-fleche-droite"></i>'],
    ...
    $(".slider-galerie").length > 0 && $(".slider-galerie").owlCarousel({
        nav: !0,
        loop: !0,
        autoWidth: !0,
        navText: ['<i class="icon-fleche-gauche"></i>', '<i class="icon-fleche-droite"></i>'],
    ...
    $(".slider-infos").length > 0 && ($(".une-info .cercle").each(function () {
        $(this).html().indexOf("%") > 0 && (szNewHTML = $(this).html().replace("%", "<span class='petit'>%</span>"), $(this).html(szNewHTML))
    }), initializeOwlInfos(), check_slider_infos(), $(window).resize(function () {
        check_slider_infos()
    }))
}), $(document).ready(function () {
    $(".slider").length > 0 && $(".slider").owlCarousel({
        nav: !0,
        items: 1,
        dots: !0,
        loop: !0,
    ...
    $(".masonry").length > 0 && (initializeOwlMasonry(), check_slider_masonry(), $(window).resize(function () {
        check_slider_masonry()
    }))
});